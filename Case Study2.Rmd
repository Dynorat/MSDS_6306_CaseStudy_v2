...
diff.temp$Country <- max.temp[1]

# Sort by largest difference
diff.temp <- diff.temp[order(-diff.temp$x),]

high.temp.variation <- diff.temp[1:20,]
```
...
diff.CityTemp$City <- max.CityTemp[1]

# Sort by largest difference
diff.CityTemp <- diff.CityTemp[order(-diff.CityTemp$x),]

high.CityTemp.variation <- diff.CityTemp[1:20,]
```
...
ggplot(dat,aes(x=x,y=dat$Country))+geom_bar() + geom_text(aes(label=dat$Country), size=3)

ggplot(dat, aes(x,x.1, color=)) + geom_point()
