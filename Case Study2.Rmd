...
```{r}
xspace<-seq(1,20,by=1)
#makeshift spacing to see Temps, need to add country labels
# ggplot(high.temp.variation, aes(xspace,Temp)) + geom_point()+ xlab("Country") + geom_text(aes(label=high.temp.variation$Country))

#failed country vs temp bar plot
...
```
```{r}
#ggplot(high.CityTemp.variation, aes(x,y=City)) + geom_bar()
# qplot(x, xspace, data=high.CityTemp.variation,geom="point", xlab="City", ylab="Temperature")
